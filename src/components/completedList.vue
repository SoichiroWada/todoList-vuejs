<template>
  <div class="completed-list">
    <h1 id="completedListTitle" v-bind:style="this.styleChange7">{{ title }}</h1>
    ...
    <div id="todo-input">
      <form>
        ...
        <button id="routingD" type="button" v-on:click="routingToDeleted">{{ routingD }}</button>
      </form>
    </div>
  ...
  <table id="todo-table">
    <tr>
      ...
      <th id="completedDateHeader">completed
        <i class="material-icons edit" id="deletedDropDown" v-on:click="sortingByCompleted(1)">arrow_drop_down</i>
        <i class="material-icons edit" id="deletedDropUp" v-on:click="sortingByCompleted(0)">arrow_drop_up</i>
      </th>
    ...
    <tr v-for="(todo, index) in todos" v-bind:key="index">
      ...
      <td id ="completedDateAndTime">{{todo.completed}}</td>
...
export default {
  ...
  methods: {
    renderCompletedLists(){
      db.collection('completedlists').get()
      .then(snapshot => {
    ...
    expout(){
      ...
      this.styleChange7.push({color:'green'});
    ...
    sortingByCompleted(value){
      ...
      .then(snapshot => {
          snapshot.forEach(doc => {
            ...
            let completedTimeObj = new Date(todoSingleList.completed);
            let milliTime = completedTimeObj.getTime();
            todoSingleList.completedSort = milliTime;

            todoSingleList.due = this.dateFormatConverter(todoSingleList.due);
            todoSingleList.created = this.dateFormatConverter(todoSingleList.created);
            todoSingleList.completed = this.dateFormatConverter(todoSingleList.completed);
        ...
        if (value === 0) {
            this.todos.sort(function (a,b){
            return a.completedSort - b.completedSort;
          })
        }
        else if (value === 1){
            this.todos.sort(function (a,b){
            return b.completedSort - a.completedSort;
        })
        }
      })
    },
    ...
    routingToEditView(todo){
      ...
      this.$router.push({name:'completedListEdit', params: { id: todo.id }});
    },
    clickedStars(value){
      // console.log(value)
      switch (true) {
    ...
    clearAllData(){
      this.title = 'Completed items List',
      this.item = null,
      this.priority = null,
      this.due = null,
      this.created = null,
    ...
    AddTodoList(){
      ...
      .then(() => {
        ...
        db.collection('completedlists').doc(this.id).delete()
          .then(() => {
    ...
    DeleteTodo(todo){
        db.collection('completedlists').doc(todo.id).delete()
          .then(() => {
  ...
  created(){
    ...
    .then(snapshot => {
        snapshot.forEach(doc => {
          let completedList = doc.data()
          // console.log('doc.data():',doc.data())
          completedList.due = this.dateFormatConverter(completedList.due);
          completedList.created = this.dateFormatConverter(completedList.created)
          completedList.completed = this.dateFormatConverter(completedList.completed)
          // console.log('completedList.completed:',completedList.completed)
          completedList.id = doc.id
          this.todos.push(completedList)

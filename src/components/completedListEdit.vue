<template>
  <div class="completed-list-edit">
    <h1 id="completedListEditTitle">{{ title }}</h1>
    <h3 id="completedListEditMessage">{{ message }}</h3>
    <div id="todo-input">
      <form v-on:submit.prevent="UpdateTodoList">
        ...
        <button id="clear" type="button" v-on:click="clearAndRouting">{{ clear }}</button>
        <button id="submit" type="submit">{{ submit }}</button>
        <button id="routingC" type="button" v-on:click="routingToCompleted">{{ routing }}</button>
      </form>
    </div>
...
export default {
  ...
  data: function(){
    return {
      title: 'Completed List - Edit Mode',
      clear: 'Clear',
      submit: 'Submit',
      routing: 'Cancel',
    ...
    UpdateTodoList(){
      ...
      db.collection('todolists').add({
          ...
          db.collection('completedlists').doc(this.id).delete()
            .then(() => {
              console.log("This doc is deleted from completedlists and added to todolists")
            })
          this.clearAllData();
  ...
  created(){
    ...
    db.collection('completedlists').doc(autoid).get()
    .then((doc) => {
...
#completedListEditMessage{
  color: aqua;
}
#completedListEditTitle{
  color: green;
}
</style>

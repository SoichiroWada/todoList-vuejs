  ...
  <div class="edit-view">
    ...
    <div id="todo-input">
      <form v-on:submit.prevent="UpdateTodoList">
        ...
        <button id="routingToTodo" type="button" v-on:click="routingToTodoList">{{ routing }}</button>
      </form>
    </div>
...
export default {
  ...
  data: function(){
    return {
      ...
      todos:[],
      deletedTodos:[],
      message: "Please edit and click Update button",
      olddue: null,
      prioritySort: null,
      dueSort: null,
      createdSort: null,
  ...
  methods: {
    ...
    UpdateTodoList(){
      ...
      db.collection('todolists').doc(this.id).set({
          item: this.item,
          ...
          //console.log("Edited data is updated !")
          this.clearAllData();
  ...
  created(){
    ...
    db.collection('todolists').doc(key).get()
    .then((doc) => {
